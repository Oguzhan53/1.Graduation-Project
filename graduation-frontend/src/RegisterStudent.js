...
class RegisterStudent extends React.Component{
  ...
  handleSubmit = (e) => {
    ...
    const { username , password , passwordRepeat,name,number,surname,classNum,department}=this.state;
    ...
    fetch("http://localhost:8083/api/admin/student",{
      ...
      credentials:"include",
      body: JSON.stringify({username,password,name,number,surname,classNum,department}),
    })
    ...
    .then((r) =>{
      ...
      if( r.status===500){
        return Promise.reject(new Error("Duplicate entry.Plese change username or number"));
      }
